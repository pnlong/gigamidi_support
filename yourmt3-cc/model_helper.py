...
def load_model_checkpoint(args=None, device='cpu'):
    ...
    parser.add_argument('-p', '--project', type=str, default='2024', help='project name')
...
def transcribe(model, audio_info, program=None, is_drum=False, write=True):
    ...
    # Inference
    pred_token_arr, _ = model.inference_file(bsz=4, audio_segments=audio_segments)
    ...
    if program is not None:
        print(f"forcing {program}")
        for n in pred_notes:
            n.program = program
            n.is_drum = is_drum


    #def save_to_midi(pred_notes, model, audio_info):
    # Write MIDI
    midifile = None
    if write:
...
def transcribe(model, audio_info):
    pred_notes = transcribe_internal(model, audio_info)
    save_to_midi(pred_notes, model, audio_info)
'''

def transcribe_single_inst(model, audio_info, program=None, is_drum=False):
    # -------------------------
    ...
    # Merge all decoding channels → single note list
    pred_note_multi = mix_notes(pred_notes_in_file)

    # -------------------------
    # 4. Assign program & is_drum if provided
    # -------------------------
    if program is not None:
        for n in pred_note_multi:
            n.program = program
            n.is_drum = is_drum

    pred_notes = pred_note_multi
    ...
    for n in pred_notes:
        prog = getattr(n, "program", 0)
        program_counts[prog] += 1
        program_is_drum[prog] = getattr(n, "is_drum", False)

    # Notes per program as percentages
    program_percentages = {
        prog: (count / total_notes) if total_notes > 0 else 0.0
        for prog, count in program_counts.items()
    }

    stats = {
        "num_programs": len(program_counts),
        "total_notes": total_notes,
        "program_note_counts": dict(program_counts),
        "program_note_percent": program_percentages,
        "is_drum_flags": program_is_drum,
    }

    return midifile, pred_notes, stats